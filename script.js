...
// ASL DOM Elements
const aslPopup = document.getElementById('asl-interpreter');
const btnCloseAsl = document.getElementById('btn-close-asl');
const aslSubtitleText = document.getElementById('asl-subtitle-text');
let aslTimeout;
...
function updateLanguage(lang) {
    ...
    // Handle ASL Popup
    if (lang === 'asl') {
        aslPopup.classList.remove('hidden');
        document.body.classList.add('asl-active');
        aslSubtitleText.textContent = "Hi I'm an AI ASL interpreter and I will be interpreting for you today.";
    } else {
        aslPopup.classList.add('hidden');
        document.body.classList.remove('asl-active');
    }
...
// --- ASL Global Click Interpreter ---
btnCloseAsl.addEventListener('click', () => {
    languageSelect.value = 'english';
    updateLanguage('english');
});

document.body.addEventListener('click', (e) => {
    // Only intercept if ASL is active
    if (!document.body.classList.contains('asl-active')) return;

    // Do not intercept clicks on the popup itself, or the language dropdown
    if (e.target.closest('.asl-popup') || e.target.closest('#language-select')) return;

    const textToInterpret = e.target.innerText || e.target.textContent || e.target.placeholder;
    
    if (textToInterpret && textToInterpret.trim().length > 0) {
        // We prevent default so links or buttons don't navigate away while inspecting
        // but only if it's not the main generate button or textarea typing
        if(e.target.tagName !== "TEXTAREA" && e.target.tagName !== "INPUT" && e.target.id !== "btn-generate") {
            e.preventDefault();
            e.stopPropagation();
        }

        aslSubtitleText.textContent = textToInterpret.trim();
        aslPopup.classList.add('asl-translating');

        // Estimate animation time based on text length (min 1.5s, max 4s)
        const duration = Math.min(Math.max(textToInterpret.length * 50, 1500), 4000);
        
        clearTimeout(aslTimeout);
        aslTimeout = setTimeout(() => {
            aslPopup.classList.remove('asl-translating');
            if (aslSubtitleText.textContent === textToInterpret.trim()) {
                aslSubtitleText.textContent = "Waiting...";
            }
        }, duration);
    }
}, { capture: true }); // using capture phase to catch before local handlers
